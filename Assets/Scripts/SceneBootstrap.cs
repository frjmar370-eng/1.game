...
public class SceneBootstrap : MonoBehaviour
{
    ...
    void BuildStartScreen(){
        ...
        Button play=CreateButton(canvas.transform,"PLAY",new Vector2(.5f,.42f),360,100);play.interactable=true;play.onClick.AddListener(StartGame);Text hint=CreateLabel(canvas.transform,"Third-person tactical arena",new Vector2(.5f,.27f),24);hint.color=new Color(.55f,.62f,.72f);Text stamp=CreateLabel(canvas.transform,BuildStamp.Version,new Vector2(.5f,.12f),18);stamp.color=new Color(.35f,.42f,.50f);
    }
    void StartGame(){
    ...
    void StartGameAfterContent(){
        if(this==null)return;BuildPlayer();BuildHUD();GameFlow flow=FindObjectOfType<GameFlow>();if(flow==null){GameObject flowObject=new GameObject("Game Flow");flow=flowObject.AddComponent<GameFlow>();}flow.BeginGame();
    ...
    void BuildPlayer(){
        if(FindObjectOfType<ShotgunGame>()!=null)return;GameObject player=new GameObject("Player");player.transform.position=new Vector3(0,0,-8f);CharacterController cc=player.AddComponent<CharacterController>();cc.height=1.8f;cc.radius=.35f;cc.center=new Vector3(0,.9f,0);cc.stepOffset=.35f;cc.slopeLimit=50f;cc.skinWidth=.04f;cc.minMoveDistance=.001f;
        GameObject visualPrefab=ArtAssetResolver.Player();if(visualPrefab){GameObject visual=Instantiate(visualPrefab,player.transform);visual.name="Player Visual";visual.transform.localPosition=Vector3.zero;visual.transform.localRotation=Quaternion.identity;NormalizeVisual(visual,1.8f);}Camera cam=new GameObject("Third Person Camera").AddComponent<Camera>();cam.tag="MainCamera";cam.fieldOfView=68;cam.nearClipPlane=.05f;cam.farClipPlane=90f;
        ShotgunGame game=player.AddComponent<ShotgunGame>();game.playerCamera=cam;game.thirdPerson=true;ShotgunWeaponView weapon=player.AddComponent<ShotgunWeaponView>();weapon.playerCamera=cam;weapon.thirdPerson=true;
    }
    void BuildHUD(){
        if(GameObject.Find("HUD Canvas")!=null)return;GameObject canvasObj=new GameObject("HUD Canvas");Canvas canvas=canvasObj.AddComponent<Canvas>();canvas.renderMode=RenderMode.ScreenSpaceOverlay;CanvasScaler scaler=canvasObj.AddComponent<CanvasScaler>();scaler.uiScaleMode=CanvasScaler.ScaleMode.ScaleWithScreenSize;scaler.referenceResolution=new Vector2(1920,1080);canvasObj.AddComponent<GraphicRaycaster>();

...
public class SceneBootstrap : MonoBehaviour
{
    ...
    void Awake(){BuildArena();BuildProps();BuildTargets();BuildPlayer();BuildLighting();BuildHUD();BuildFlow();}
    ...
    void BuildPlayer(){
        GameObject player=new GameObject("Player");player.transform.position=new Vector3(0,1.2f,-7f);CharacterController cc=player.AddComponent<CharacterController>();cc.height=1.8f;cc.radius=.35f;
        ...
        ShotgunGame game=player.AddComponent<ShotgunGame>();game.playerCamera=cam;ShotgunWeaponView weapon=player.AddComponent<ShotgunWeaponView>();weapon.playerCamera=cam;
    }
    ...
    void BuildHUD(){
        ...
        MobileHUD mobile=MobileHUD.Build(canvas);ShotgunGame game=FindObjectOfType<ShotgunGame>();if(game!=null){game.ammoText=ammo;game.scoreText=score;game.healthText=hp;game.messageText=message;game.mobileHUD=mobile;}GameFlow flow=FindObjectOfType<GameFlow>();if(flow!=null){flow.roundText=round;flow.messageText=message;}
    }

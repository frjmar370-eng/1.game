...
public class SceneBootstrap : MonoBehaviour
{
    ...
    void BuildStartScreen()
    {
        ...
        Image background = CreatePanel(canvas.transform, new Color(.035f, .045f, .06f, 1f));
        background.rectTransform.anchorMin = Vector2.zero;
        background.rectTransform.anchorMax = Vector2.one;
        background.rectTransform.offsetMin = Vector2.zero;
        background.rectTransform.offsetMax = Vector2.zero;

        Text title = CreateLabel(canvas.transform, "SHOTGUN 3D", new Vector2(.5f, .68f), 82);
        title.fontStyle = FontStyle.Bold;
        Text subtitle = CreateLabel(canvas.transform, "TARGET RANGE", new Vector2(.5f, .59f), 30);
        subtitle.color = new Color(.75f, .78f, .82f);

        Button play = CreateButton(canvas.transform, "PLAY", new Vector2(.5f, .42f), 360, 100);
        play.onClick.AddListener(StartGame);

        Text hint = CreateLabel(canvas.transform, "Tap PLAY to start", new Vector2(.5f, .27f), 24);
        hint.color = new Color(.65f, .68f, .72f);
    }
    ...
    void StartGame()
    {
        ...
        GameObject menu = GameObject.Find("Start Screen Canvas");

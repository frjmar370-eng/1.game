...
public class ShotgunWeaponView : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(prefab!=null)
        {
            ...
            if(thirdPerson && rightHand!=null)
            {
                weapon.SetParent(rightHand,false);
                weapon.localPosition=new Vector3(.03f,-.03f,.02f);
                weapon.localRotation=Quaternion.Euler(0,90,0);
                handRestLocal=weapon.localPosition;
            }
            else if(thirdPerson)
            {
                weapon.localPosition=restPos;weapon.localRotation=Quaternion.identity;
                handRestLocal=restPos;
            }
            else if(playerCamera)
            {
                weapon.SetParent(playerCamera.transform,false);weapon.localPosition=new Vector3(.32f,-.28f,.55f);weapon.localRotation=Quaternion.Euler(4,-8,0);handRestLocal=weapon.localPosition;
            }
    ...
    {
        if(!weapon)return;
        recoil=Mathf.MoveTowards(recoil,0,Time.deltaTime*4f);
        weapon.localPosition=handRestLocal+Vector3.back*recoil;
    }

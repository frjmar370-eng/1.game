...
public class ShotgunWeaponView : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(prefab!=null)
        {
            ...
            if(thirdPerson){weapon.localPosition=restPos;weapon.localRotation=Quaternion.identity;NormalizeImportedModel(weapon,.95f);}
            else if(playerCamera){weapon.SetParent(playerCamera.transform,false);weapon.localPosition=new Vector3(.32f,-.28f,.55f);weapon.localRotation=Quaternion.Euler(4,-8,0);NormalizeImportedModel(weapon,.95f);}
        }
    ...
    void Update(){if(!weapon)return;recoil=Mathf.MoveTowards(recoil,0,Time.deltaTime*4f);weapon.localPosition=restPos+Vector3.back*recoil;}
    ...
    Transform FindDeepChild(Transform root,params string[] names){if(root==null)return null;foreach(Transform t in root.GetComponentsInChildren<Transform>(true)){string n=t.name.ToLowerInvariant().Replace(" ","").Replace("-","_");foreach(string wanted in names)if(n.Contains(wanted))return t;}return null;}

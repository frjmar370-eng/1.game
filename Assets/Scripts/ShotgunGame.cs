...
public class ShotgunGame : MonoBehaviour
{
    ...
    float nextFire; int score; int health=100; float yaw,pitch; bool reloading; float reloadDone; bool gameOver; ShotgunWeaponView weapon; CharacterController controller;
    Vector3 cameraVelocity;

    void Start(){weapon=GetComponent<ShotgunWeaponView>();controller=GetComponent<CharacterController>();if(playerCamera)playerCamera.transform.position=transform.position+new Vector3(0,2.4f,-5f);UpdateUI();}
    ...
    void Update()
    {
        ...
        Look(); Move();
        bool fire=Input.GetMouseButton(0)||(mobileHUD&&mobileHUD.FireHeld); if(fire)Fire();
    ...
    void Look()
    {
        ...
        pitch=Mathf.Clamp(pitch,-35f,55f);
        ...
        if(thirdPerson)
        {
            ...
            Vector3 desired=target+orbit*new Vector3(0,0,-5.2f);
            Vector3 origin=target; Vector3 dir=desired-origin; float dist=dir.magnitude;
            if(Physics.SphereCast(origin,.22f,dir.normalized,out RaycastHit hit,dist,~0,QueryTriggerInteraction.Ignore)) desired=origin+dir.normalized*Mathf.Max(1.1f,hit.distance-.18f);
            playerCamera.transform.position=Vector3.SmoothDamp(playerCamera.transform.position,desired,ref cameraVelocity,.06f);
            playerCamera.transform.rotation=Quaternion.LookRotation((target+Vector3.up*(pitch*.012f))-playerCamera.transform.position,Vector3.up);
        }
    ...
    void Move()
    {
        ...
        Vector3 move=(transform.right*x+transform.forward*z); if(move.sqrMagnitude>1f)move.Normalize(); move*=moveSpeed*Time.deltaTime;
        if(controller){if(!controller.isGrounded)move.y-=9.81f*Time.deltaTime;controller.Move(move);}else transform.position+=move;
    }

    public void Fire()
    {
        if(gameOver||reloading||Time.time<nextFire||ammo<=0||playerCamera==null)return;
        nextFire=Time.time+fireCooldown;ammo--;if(weapon)weapon.FireKick();
        Vector3 aimOrigin=playerCamera.transform.position; Vector3 aimDir=playerCamera.transform.forward;
        for(int i=0;i<pellets;i++){
            Vector3 direction=aimDir+Random.insideUnitSphere*spread;
            if(Physics.Raycast(aimOrigin,direction.normalized,out RaycastHit hit,range,~0,QueryTriggerInteraction.Ignore)){

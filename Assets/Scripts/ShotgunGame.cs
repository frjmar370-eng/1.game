...
public class ShotgunGame : MonoBehaviour
{
    ...
    void Start(){weapon=GetComponent<ShotgunWeaponView>();controller=GetComponent<CharacterController>();animator=GetComponentInChildren<Animator>();idleHash=Animator.StringToHash("Idle");walkHash=Animator.StringToHash("Walk");runHash=Animator.StringToHash("Run");if(GameSettings.Instance)lookSensitivity=GameSettings.Instance.sensitivity;if(playerCamera)playerCamera.transform.position=transform.position+new Vector3(0,2.4f,-5f);if(CombatFeedback.Instance==null)new GameObject("Combat Feedback").AddComponent<CombatFeedback>();UpdateUI();}
    ...
    void Look(){Vector2 look=mobileHUD?mobileHUD.Look:Vector2.zero;if(look.sqrMagnitude>.001f){yaw+=look.x*lookSensitivity*2.2f;pitch-=look.y*lookSensitivity*2.2f;}else if(Input.touchCount==1){Touch t=Input.GetTouch(0);if(t.position.x>Screen.width*.35f&&t.phase==TouchPhase.Moved){yaw+=t.deltaPosition.x*lookSensitivity*.08f;pitch-=t.deltaPosition.y*lookSensitivity*.08f;}}else{yaw+=Input.GetAxis("Mouse X")*lookSensitivity;pitch-=Input.GetAxis("Mouse Y")*lookSensitivity;}pitch=Mathf.Clamp(pitch,-35f,55f);transform.rotation=Quaternion.Euler(0,yaw,0);if(!playerCamera)return;if(thirdPerson){Vector3 target=transform.position+Vector3.up*1.25f;Quaternion orbit=Quaternion.Euler(pitch,yaw,0);Vector3 desired=target+orbit*new Vector3(0,0,-5.2f);Vector3 dir=desired-target;float dist=dir.magnitude;if(Physics.SphereCast(target,.22f,dir.normalized,out RaycastHit hit,dist,~0,QueryTriggerInteraction.Ignore))desired=target+dir.normalized*Mathf.Max(1.1f,hit.distance-.18f);playerCamera.transform.position=Vector3.SmoothDamp(playerCamera.transform.position,desired,ref cameraVelocity,.06f);playerCamera.transform.rotation=Quaternion.LookRotation((target+Vector3.up*(pitch*.012f))-playerCamera.transform.position,Vector3.up);}else playerCamera.transform.localRotation=Quaternion.Euler(pitch,0,0);}
    ...
    public void Fire(){if(gameOver||reloading||Time.time<nextFire)return;if(ammo<=0){if(GameSettings.Instance==null||GameSettings.Instance.autoReload)Reload();return;}nextFire=Time.time+fireCooldown;ammo--;if(weapon)weapon.FireKick();if(CombatFeedback.Instance)CombatFeedback.Instance.Shot();Vector3 aimOrigin=playerCamera.transform.position;Vector3 aimDir=playerCamera.transform.forward;for(int i=0;i<pellets;i++){Vector3 direction=aimDir+Random.insideUnitSphere*spread;if(Physics.Raycast(aimOrigin,direction.normalized,out RaycastHit hit,range,~0,QueryTriggerInteraction.Ignore)){TargetDummy target=hit.collider.GetComponentInParent<TargetDummy>();if(target!=null){bool killed=target.Hit(1);score+=killed?100:5;if(CombatFeedback.Instance)CombatFeedback.Instance.Hit();}}}UpdateUI();if(ammo==0&&(GameSettings.Instance==null||GameSettings.Instance.autoReload))Reload();}
    public void Reload(){if(gameOver||reloading||ammo==maxAmmo)return;reloading=true;reloadDone=Time.time+1.15f;if(messageText)messageText.text="RELOADING...";UpdateUI();}
    public void AddAmmo(int amount){ammo=Mathf.Min(maxAmmo,ammo+Mathf.Max(1,amount));if(messageText)messageText.text="AMMO +"+amount;UpdateUI();}
    ...
    public void TakeDamage(int amount){if(gameOver)return;health=Mathf.Max(0,health-amount);if(CombatFeedback.Instance)CombatFeedback.Instance.Shake(.12f);UpdateUI();if(health==0){gameOver=true;if(messageText)messageText.text="GAME OVER";if(mobileHUD)mobileHUD.ShowRestart();}}

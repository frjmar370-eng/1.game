...
public class ShotgunGame : MonoBehaviour
{
    ...
    void UpdateAnimation(float amount){if(!animator)return;int hash=amount<.05f?idleHash:(amount>.7f?runHash:walkHash);if(animator.HasState(0,hash))animator.CrossFade(hash,.12f);}
    public void Fire(){
        ...
        if(w==0)for(int i=0;i<pellets;i++)HitScan(origin,aim+Random.insideUnitSphere*spread,1);else if(w==1)HitScan(origin,aim,7);else HitScan(origin,aim,12);

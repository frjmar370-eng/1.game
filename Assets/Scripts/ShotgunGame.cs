...
public class ShotgunGame : MonoBehaviour
{
    ...
    public void Fire(){
        ...
        if(weapons!=null){if(!weapons.Fire()){if(weapons.Ammo<=0&&weapons.Reserve>0)Reload();return;}}else{if(Time.time<nextFire)return;if(ammo<=0){Reload();return;}nextFire=Time.time+fireCooldown;ammo--;}
        ...
        UpdateUI();if(weapons!=null&&weapons.Ammo<=0&&(GameSettings.Instance==null||GameSettings.Instance.autoReload)&&weapons.Reserve>0)Reload();else if(weapons==null&&ammo==0)Reload();
    }
    ...
    public void Reload(){if(gameOver||reloading)return;if(weapons!=null){if(weapons.Ammo>=weapons.magazineSize[weapons.currentWeapon]||weapons.Reserve<=0)return;}else if(ammo>=maxAmmo)return;reloading=true;reloadDone=Time.time+1.15f;if(messageText)messageText.text="RELOADING...";UpdateUI();}
    ...
    void UpdateUI(){if(weapons!=null){if(ammoText)ammoText.text=weapons.weaponNames[weapons.currentWeapon]+"  "+weapons.Ammo+"/"+weapons.Reserve;}else if(ammoText)ammoText.text="Shotgun  "+ammo+"/"+maxAmmo;if(scoreText)scoreText.text="SCORE  "+score;if(healthText)healthText.text="HP  "+health;}
}

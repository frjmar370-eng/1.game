...
public class ShotgunGame : MonoBehaviour
{
    ...
    public void Fire(){if(gameOver||reloading||Time.time<nextFire||ammo<=0||playerCamera==null)return;nextFire=Time.time+fireCooldown;ammo--;if(weapon)weapon.FireKick();if(CombatFeedback.Instance)CombatFeedback.Instance.Shot();Vector3 aimOrigin=playerCamera.transform.position;Vector3 aimDir=playerCamera.transform.forward;for(int i=0;i<pellets;i++){Vector3 direction=aimDir+Random.insideUnitSphere*spread;if(Physics.Raycast(aimOrigin,direction.normalized,out RaycastHit hit,range,~0,QueryTriggerInteraction.Ignore)){TargetDummy target=hit.collider.GetComponentInParent<TargetDummy>();if(target!=null){bool killed=target.Hit(1);score+=killed?100:5;if(CombatFeedback.Instance)CombatFeedback.Instance.Hit();}}}UpdateUI();}

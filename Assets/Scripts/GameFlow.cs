...
public class GameFlow : MonoBehaviour
{
    ...
    void Awake(){
        ...
        Instance=this;
        enemyPrefab=ArtAssetResolver.Enemy();
    }
    ...
    public void BeginGame(){
        ...
        SpawnRoundTargets(6);
        SpawnPickups(4);
        Invoke(nameof(RefreshEnemyPresentation),.2f);
        UpdateUI();
    }
    ...
    void Update(){
        ...
        bool bossBlocking=BossSystem.Instance!=null&&BossSystem.Instance.Active;
        if(timeLeft<=0f||(!bossBlocking&&aliveTargets<=0))BeginNextRound();
        UpdateUI();
    }
    ...
    public void TargetKilled(){
        ...
        bool bossBlocking=BossSystem.Instance!=null&&BossSystem.Instance.Active;
        if(aliveTargets==0&&!bossBlocking&&!transitioning)BeginNextRound();
    }
    ...
    void EndRound(){
        ...
        if(messageText)messageText.text=round%5==0?"BOSS WAVE":"ROUND "+round;
        if(AudioManager.Instance)AudioManager.Instance.Click();
    }

    void SpawnRoundTargets(int count){
        aliveTargets=count;
        float healthMul=DifficultySystem.Instance?DifficultySystem.Instance.enemyHealth:1f;
        float speedMul=DifficultySystem.Instance?DifficultySystem.Instance.enemySpeed:1f;
        float damageMul=DifficultySystem.Instance?DifficultySystem.Instance.enemyDamage:1f;
        for(int i=0;i<count;i++){
            ...
            d.moveSpeed=(1.2f+round*.08f)*speedMul;
            d.damageToPlayer=Mathf.RoundToInt((8+Mathf.Min(14,round))*damageMul);
            d.attackInterval=Mathf.Max(.65f,2.5f-round*.08f)/speedMul;
            d.chaseDistance=Mathf.Min(32f,18f+round*.5f);
            EnemyVariants v=t.GetComponent<EnemyVariants>();if(v==null)v=t.AddComponent<EnemyVariants>();
            int roll=(i+round)%10;v.type=roll<6?EnemyType.Grunt:(roll<9?EnemyType.Runner:EnemyType.Tank);
            if(!t.GetComponent<EnemyVisualPolish>())t.AddComponent<EnemyVisualPolish>();
            CombatPresentation3D cp=FindObjectOfType<CombatPresentation3D>();if(cp)cp.Attach(d);
        }
    }

    void SpawnPickups(int count){
        for(int i=0;i<count;i++){
            GameObject prefab=i%3==0?ArtAssetResolver.Crate():null;
            GameObject o=prefab?Instantiate(prefab):GameObject.CreatePrimitive(PrimitiveType.Sphere);
            o.name="Pickup_"+round+"_"+i;
            o.transform.position=new Vector3(Random.Range(-14f,14f),prefab?.6f:.65f,Random.Range(-7f,17f));
            if(!prefab)o.transform.localScale=Vector3.one*.38f;
            Collider old=o.GetComponent<Collider>();if(old)Destroy(old);
            SphereCollider trigger=o.AddComponent<SphereCollider>();trigger.isTrigger=true;trigger.radius=.6f;
            PickupItem p=o.GetComponent<PickupItem>();if(!p)p=o.AddComponent<PickupItem>();
            p.kind=(i%3==0)?PickupItem.Kind.Health:PickupItem.Kind.Ammo;p.amount=p.kind==PickupItem.Kind.Health?20:2;
        }
    }

    void RefreshEnemyPresentation(){
        CombatPresentation3D cp=FindObjectOfType<CombatPresentation3D>();if(cp==null)return;
        foreach(TargetDummy d in FindObjectsOfType<TargetDummy>()){cp.Attach(d);if(!d.GetComponent<EnemyVisualPolish>())d.gameObject.AddComponent<EnemyVisualPolish>();}
    }

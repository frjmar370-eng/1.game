...
public class GameFlow : MonoBehaviour
{
    ...
    void SpawnRoundTargets(int count){
        ...
        for(int i=0;i<count;i++){
            ...
            TargetDummy d=t.GetComponent<TargetDummy>();
            if(d==null)d=t.AddComponent<TargetDummy>();
            ...
            EnemyVariants v=t.GetComponent<EnemyVariants>();
            if(v==null)v=t.AddComponent<EnemyVariants>();
            int roll=(i+round)%10;
            v.type=roll<6?EnemyType.Grunt:(roll<9?EnemyType.Runner:EnemyType.Tank);
        }
    }
    ...
    void SpawnPickups(int count){
        for(int i=0;i<count;i++){
            ...
            o.transform.position=new Vector3(Random.Range(-10f,10f),.65f,Random.Range(-5f,10f));
            o.transform.localScale=Vector3.one*.38f;
            Destroy(o.GetComponent<Collider>());
            SphereCollider trigger=o.AddComponent<SphereCollider>();
            trigger.isTrigger=true;
            PickupItem p=o.AddComponent<PickupItem>();
            p.kind=(i%3==0)?PickupItem.Kind.Health:PickupItem.Kind.Ammo;
            p.amount=p.kind==PickupItem.Kind.Health?20:2;
        }
    }

    void UpdateUI(){
        if(roundText)roundText.text="ROUND  "+round+"   "+Mathf.CeilToInt(Mathf.Max(0,timeLeft))+"   ENEMIES "+aliveTargets;
    }
}

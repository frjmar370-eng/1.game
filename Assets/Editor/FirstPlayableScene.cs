...
public static class FirstPlayableScene
{
    ...
    public static void EnsureScene(){
        ...
        RenderSettings.ambientLight=new Color(.16f,.19f,.24f);
        RenderSettings.fog=true; RenderSettings.fogColor=new Color(.035f,.045f,.06f); RenderSettings.fogDensity=.004f;
        ...
        var hero=Object.Instantiate(heroPrefab); hero.name="PLAYER_3D"; hero.transform.position=new Vector3(0,0.2f,-20); hero.transform.localScale=Vector3.one;
        var cc=hero.AddComponent<CharacterController>(); cc.height=3.8f; cc.radius=.55f; cc.center=new Vector3(0,1.9f,0);
        ...
        var camObj=new GameObject("Main Camera"); var cam=camObj.AddComponent<Camera>(); cam.tag="MainCamera"; cam.fieldOfView=62; cam.nearClipPlane=.1f; cam.farClipPlane=220;
        ...
        var light=new GameObject("Sun"); var dl=light.AddComponent<Light>(); dl.type=LightType.Directional; dl.intensity=1.15f; dl.shadows=LightShadows.Soft; light.transform.rotation=Quaternion.Euler(48,-32,0);
        CreateHUD();
        EditorSceneManager.SaveScene(scene,ScenePath);
        AssetDatabase.SaveAssets();
        Debug.Log("SHOTGUN 3D: first playable city scene created with authored OBJ assets.");
    ...
    static void CreateHUD(){
        ...
        Text title=Text("SHOTGUN 3D",new Vector2(55,-38),new Vector2(400,70),44,FontStyle.Bold); title.color=Color.white;
        Text mission=Text("MISSION 01  •  دخول المنطقة",new Vector2(55,-95),new Vector2(560,55),24,FontStyle.Bold); mission.color=new Color(.35f,.85f,1);
        ...
        Image cross=Ring(new Vector2(0,0),new Vector2(70,70)); cross.color=new Color(1,1,1,.75f);
    ...
    static Image Ring(Vector2 pos,Vector2 size){return Panel(pos,size,new Color(1,1,1,.18f),"Crosshair");}
}
#endif
